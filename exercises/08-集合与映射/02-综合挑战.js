 ...
 * 模块 08: 集合与映射 - 综合挑战
 *
 * 提示: 遇到困难时可以查看 solutions/08-集合与映射/02-综合挑战.solution.js
 */
...
describe("模块08 - 集合与映射综合挑战", () => {
  ...
  test("挑战1: 完整的 Set 工具库", () => {
    ...
    expect([...SetUtils.symmetricDifference(setA, setB)]).toEqual([???]);
  });
  ...
  test("挑战3: 多值 Map", () => {
    ...
    expect(mmap.get("fruit").size).toBe(???);
    expect(mmap.has("fruit", "apple")).toBe(true);
  });

  test("挑战4: 有序 Map", () => {
    // TODO: 实现 OrderedMap 类，保持插入顺序
    class OrderedMap {
      constructor() {
        this.keys = [];
        this.values = [];
