 ...
 * 模块 06: 对象操作 - 综合挑战
 *
 * 提示: 遇到困难时可以查看 solutions/06-对象操作/02-综合挑战.solution.js
 */

import { test, expect, describe } from "bun:test";

describe("模块06 - 对象综合挑战", () => {

  test("挑战1: 深度克隆", () => {
    // TODO: 实现 deepClone 函数，支持嵌套对象、数组和 Date
    function deepClone(obj) {
      if (obj === null || typeof obj !== "object") return obj;
      if (obj instanceof Date) return new Date(obj);
      if (obj instanceof Array) return obj.map(item => ???);
  ...
  test("挑战5: 对象不可变更新", () => {
    ...
    function update(obj, path, updater) {
      ...
      return {
        ...obj,
        [key]: update(???, keys.slice(1), updater)
      };
    }
  ...
  test("挑战6: 对象观察者", () => {
    // TODO: 实现 ObservableObject 类，支持订阅和通知
  ...
  test("挑战7: 对象验证", () => {
    // TODO: 实现 validate 函数，根据 schema 验证对象
    ...
    function flatten(obj, prefix = "") {
      ...
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (value && typeof value === "object" && !Array.isArray(value)) {
          Object.assign(result, flatten(???, newKey));
        } else {
          result[newKey] = value;
        }
      }
